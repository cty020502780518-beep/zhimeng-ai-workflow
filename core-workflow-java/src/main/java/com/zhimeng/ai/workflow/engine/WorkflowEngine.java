package com.zhimeng.ai.workflow.engine;

import com.zhimeng.ai.workflow.engine.constants.EndNodeOutputModeEnum;
import com.zhimeng.ai.workflow.engine.constants.NodeExecStatusEnum;
import com.zhimeng.ai.workflow.engine.constants.NodeStatusEnum;
import com.zhimeng.ai.workflow.engine.constants.NodeTypeEnum;
import com.zhimeng.ai.workflow.engine.context.EngineContextHolder;
import com.zhimeng.ai.workflow.engine.domain.NodeRunResult;
import com.zhimeng.ai.workflow.engine.domain.NodeState;
import com.zhimeng.ai.workflow.engine.domain.WorkflowDSL;
import com.zhimeng.ai.workflow.engine.domain.callbacks.ChatCallBackStreamResult;
import com.zhimeng.ai.workflow.engine.domain.callbacks.LLMGenerate;
import com.zhimeng.ai.workflow.engine.domain.chain.Edge;
import com.zhimeng.ai.workflow.engine.domain.chain.Node;
import com.zhimeng.ai.workflow.engine.node.NodeExecutor;
import com.zhimeng.ai.workflow.engine.node.StreamCallback;
import com.zhimeng.ai.workflow.engine.node.callback.WorkflowMsgCallback;
import com.zhimeng.ai.workflow.engine.util.FlowUtil;
import com.zhimeng.ai.workflow.exception.ErrorCode;
import com.zhimeng.ai.workflow.exception.NodeCustomException;
            ...
            for (Edge edge : workflowDSL.getEdges()) {
                ...
                if (StringUtils.isNotBlank(handle)) {
                    if (handle.startsWith(NodeTypeEnum.CONDITION_SWITCH_NORMAL_ONE_OF.getValue())) {
                        ...
                        // 异常case的执行链路
                        // fixme 尚未完全弄清楚zhimeng_agent的异常场景，先简单实现一个，这里只处理了 intent_chain|fail_one_of 的异常场景
