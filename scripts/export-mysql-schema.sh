...
MYSQL_PORT="3306"
MYSQL_USER="root"
MYSQL_PASSWORD="${MYSQL_PASSWORD:?set MYSQL_PASSWORD first}"
...
# 导出 zhimeng_console 数据库 (包含数据)
echo -e "${YELLOW}[3/8] 导出 zhimeng_console 数据库...${NC}"
mysqldump -h"$MYSQL_HOST" -P"$MYSQL_PORT" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" \
  --databases \
  --single-transaction \
  --quick \
  --lock-tables=false \
  zhimeng_console > "$SQL_DIR/schema.sql"
...
echo -e "  - ${YELLOW}schema.sql${NC}            (zhimeng_console, 结构+数据)"
...
echo "2. schema.sql 包含 zhimeng_console 的所有配置数据"
